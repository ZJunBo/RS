...
def adv_adjust_learning_rate(optimizer, i_iter, cfg):
    if i_iter < cfg.AT_PREHEAT_STEPS:
        lr = lr_warmup(cfg.AT_LEARNING_RATE, i_iter, cfg.AT_PREHEAT_STEPS)
    else:
        lr = lr_poly(cfg.AT_LEARNING_RATE, i_iter, cfg.AT_traing_steps, cfg.POWER)
    optimizer.param_groups[0]['lr'] = lr
    if len(optimizer.param_groups) > 1:
        optimizer.param_groups[1]['lr'] = lr * 10
    return lr

def adv_adjust_learning_rate_D(optimizer, i_iter, cfg):
    if i_iter < cfg.AT_PREHEAT_STEPS:
        lr = lr_warmup(cfg.AT_LEARNING_RATE_D, i_iter, cfg.AT_PREHEAT_STEPS)
    else:
        lr = lr_poly(cfg.AT_LEARNING_RATE_D, i_iter, cfg.AT_traing_steps, cfg.POWER)
    optimizer.param_groups[0]['lr'] = lr
    if len(optimizer.param_groups) > 1:
        optimizer.param_groups[1]['lr'] = lr * 10
    return lr
